...
- **行号语义统一**：word-wrap 下前向驱动与回声抑制统一按 doc line 换算，消除折行 >2 行时的回声漏判与往返漂移
- **页顶边界**：滚回顶部可正确回带首个块；顶部块判定阈值改为按文档实测偏移，不再依赖固定 140px
- **锚点拓扑缓存**：渲染后一次采集 `offsetTop → line` 升序表，视口顶块二分定位（O(n) → O(log n)），增量渲染 / mermaid 转换后自动置脏重建
- **滚动过程跟随**：`scroll` 事件 + rAF 节流，顶部块变化即上报；`scrollend` 保留精确兜底（原先须等完全停止才跟随）
- **回声抑制双保险**：反向写回后 300ms 前向静默窗（兼方向锁）+ 既有 ±2 行目标值比对

### v1.2.2 更新

**修复（双向同步滚动抖动 / 滚不到底，菜单 3+4 同开场景）**

- **方向锁（前向）**：预览驱动（编辑器→预览）注入 JS 先打"程序性滚动"标记（目标块 + 时间戳），滚动监听器 600ms 内对目标块的回落上报直接丢弃——反馈环从源头切断；用户随即滚动预览（顶部块变化）仍实时上报
- **移除 300ms 前向静默窗**：方向锁就位后静默窗只剩"死 300ms→突跳"的抖动副作用，删除；±2 行双向回声比对保留作第二层
- **双向底部锁定**：编辑器已滚到文档尾时，预览直接对齐页面底部（不再块对齐）；预览滚到底时上报带 `;b` 标记，编辑器直达文档尾——两栏都能"顺利到底"
- **行号基准统一（前向 +1）**：前向查询统一换算为 1 基源码行，与 `data-line`、反向回写同语义，消除 0 基/1 基差一

**说明（`---` 分隔线的 CommonMark 语义）**
...
- **预览工具栏按钮显示不完整**：表单硬编码 7.8pt 小字在不同 DPI/系统字体下与 Designer 固定尺寸失配，按钮文本被裁切。现改用系统 UI 字体（`SystemFonts.MessageBoxFont`，先于 `InitializeComponent` 赋值），图标按原生 16px 绘制（不再放大发虚），工具栏显式 `AutoSize`、隐藏 Grip、按钮显式 `ImageAndText`——叠加在 5 按钮直出布局上，任何字体/缩放组合下完整显示。
